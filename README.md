...
托盘菜单：当前状态 / 显示配对码 / 输入配对码 / **已配对设备**（列出每台及
在线状态，可解除配对）/ 发送类型开关 / 大小上限 / 发送限速 / 同步端口 /
自动压缩 / 暂停同步 / 开机自启 / **详细日志** / **打开日志文件夹** / 退出。

图标颜色即状态——绿=已连接、灰=未连接、琥珀=已暂停、红=同步已停止（异常）。

大小上限与发送限速除预设档外都可**自定义录入**，接受 `500MB`、`1.5GiB`、
`20mbps` 这类写法。
