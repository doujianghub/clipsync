...
**配对也可以全程不碰命令行**：托盘菜单里，一方选「显示配对码…」，那一串
（形如 `ABCDEF@100.88.88.22`，**自带地址**）已经在他剪贴板里了；发给对方，
对方选「输入配对码…」直接粘贴即可。配对成功**立即生效，无需重启**。

**跨网络（如 Tailscale）照样不用输 IP**：覆盖网不转发组播，自动发现在那边
不管用——所以配对码里本来就带着地址。若那个地址连不通（主持方给的是覆盖网
地址而你只在局域网上），程序会自动改试局域网发现，都落空了才问你要 IP。
