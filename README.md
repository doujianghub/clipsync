...
**跨网络（如 Tailscale）**：覆盖网不转发组播，自动发现在那边不管用。主持方
的窗口里会另给一串 `ABCDEF@100.88.88.22`，把它整个粘进「输入配对码…」即可。
...
> 列表里会标出「经 某某 认识」，让你分得清哪些是自己亲手加的。从列表里
> 解除某台设备后**不会**再被引荐回来（记在 `blocked.json`），除非你重新
> 亲手与它配对。
