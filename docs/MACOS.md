...
| 内存（纯文本/文件同步） | 10–15 MB，无增长 | 12–15 MB |
| 内存（同步过图片后） | 峰值随图片尺寸抬升，见下方说明 | — |
...
### 关于内存：同步过图片后会明显抬升，但不是泄漏

跨平台联调时双方都观察到内存比基线高（Windows 侧 +5.4MB，Mac 侧一个跑了
4 小时的实例稳定在 32MB）。用全新实例做对照实验后查明：

| 场景 | RSS |
|---|---|
| 纯空闲 | 10.6 MB → 4 分钟后 9.4 MB（不升反降） |
| 读一次 200×150 图片（原始数据仅 120 KB） | 17.1 MB（**+6.3 MB**） |
| 再读同尺寸图 ×3 | 16.8 MB（**持平，不累加**） |
| 读一次 2000×1500 图片（RGBA 12 MB） | 50.6 MB |
| 再读同尺寸大图 ×2 | 50.5 MB（**仍持平**） |
| 约 10 分钟后 | 16.2 MB（**分配器已归还**） |

三点结论：

1. **不是泄漏**——重复处理同尺寸图片时 RSS 完全持平；泄漏会随次数线性上升。
2. **开销来自解码路径而非图片本身**——120 KB 小图就抬升 6.3 MB，多出来的是
   TIFF 解码 + RGBA 展开 + `image` crate 的临时缓冲。
3. **会延迟归还**——峰值不会立刻回落，约十分钟后才归还给系统。所以短时间内
   看到高位数字不代表有问题。

因此本页"内存 10–15 MB"指的是**纯文本/文件同步**场景；同步过图片后峰值取决
于图片尺寸。若看到几十 MB 且**不随图片操作次数继续增长**，属正常。

> 要压低峰值得改 `arboard` 的读图路径（避免一次性展开整张 RGBA），属独立
> 优化，收益有限——图片同步本就是低频操作。
