...
>
> **尚未验证**：真正的 Mac↔Windows 双机验证（本机只有 Mac，需要 Windows 侧
...
> ⚠️ **测性能务必用优化构建**：`cargo build --release`。debug 构建下同一个
> 90MB 传输需要 15.5 秒（5.9 MB/s），优化后是 0.6 秒——加密与哈希在 debug 下
> 慢一个数量级。用 debug 数据判断性能会得出完全错误的结论。

| 指标 | Windows 实测（release 构建） |
|---|---|
| 空闲 CPU | ≈0.1%（30 秒 0.03 秒） |
| 内存 | 12–15 MB |
| 传 90MB 文件时内存 | 恒定不涨 |
| 90MB 不可压缩文件 | 0.61 s（146 MB/s） |
| 90MB 可压缩文本 | 0.49 s（191 MB/s 等效） |
| 限速 10 MB/s 时 | 8.36 s（与设定一致） |
| 90MB 断点续传 | MD5 完全一致 |
| 重复内容秒同步 | ~1 毫秒，零传输 |
| 连接稳定性 | 长时间 0 次断开 |

Mac 上跑出来若明显偏慢，先确认是不是用了 debug 构建；其次看是否触发了
`upload_limit_bytes_per_sec` 限速。

### macOS 侧实测结果（2026-08-04，M 系列 / macOS 15.5，fast 构建）

用一台 Mac 上的两个实例（`CLIPSYNC_CONFIG_DIR` 隔离、不同 `listen_port`、
接收方加 `CLIPSYNC_NO_WATCH=1`）走真实网络栈验证，与 Windows 基线同量级：

| 指标 | macOS 实测 | Windows 基线 |
|---|---|---|
| 空闲 CPU | ≈0.03%（60 秒 0.02 秒） | ≈0.1% |
| 内存（纯文本/文件同步） | 10–16 MB，无增长 | 12–15 MB |
| 内存（同步过图片后） | 峰值≈RGBA×3，约 2 分钟归还，见下方说明 | — |
| 内存（传 2GB 文件） | ≤18 MB，与文件大小无关 | 恒定不涨 |
| 90MB 不可压缩文件 | 0.74 s（≈122 MB/s） | 0.61 s（146 MB/s） |
| 88MB 可压缩文本 | 0.55 s（≈167 MB/s 等效） | 0.49 s（191 MB/s） |
| 断点续传 | ✅ 从 49,519,581 字节续传，MD5 一致 | MD5 一致 |
| 重复内容秒同步 | 3.6 ms，零传输 | ~1 ms |
| 传输中插入文字 | ✅ 文字立即送达，文件传输被取代 | 同 |
| 配对自动发现 | ✅ `pair <配对码>` 无需 IP 即连上 | — |

双机验证清单中的第 5、6、7 项（打断、续传、缓存命中）已在单机双实例下
全部通过，日志与文档描述完全一致（`已收部分保留待续传` / `断点续传` /
`文件已在缓存中命中，秒同步（零传输）`）。

**真实双机联调已于 2026-08-04 完成**，结果见
[`CROSS_PLATFORM.md`](CROSS_PLATFORM.md)。要点：
文件名编码（中文/emoji/重音）逐字节一致——但**修复过一个 NFD 缺陷**；
路径分隔符与字节序经查证不构成风险（`FileMeta.name` 只存文件名，
帧头显式大端且两端架构均为小端）。

> **一个容易踩的坑**（已修）：`max_bytes` 默认 100 MiB，超过就会被静默跳过
> （`SkipReason::TooLarge`，只在 DEBUG 级别打日志）。测试时若用了 400MB 的
> 文件而只开 INFO 日志，会看到"两边都没反应"却找不到原因。
>
> 后来这个设置改名 `auto_fetch_bytes` 并挪到接收侧：超限的文件挂在接收方
> 托盘上等人点「取回」，不再静默消失，日志也是 INFO 级。

### 关于内存：两种不同的抬升，都不是泄漏

跨平台联调时双方都观察到内存高于基线（Windows +5.4MB，Mac 一个跑了 4 小时的
实例 32MB）。做了专项测量后查明是**两个互不相关**的原因。

> 更正：本节先前把 +6MB 归因于"图片解码"，**这是错的**。见下方第一条。

#### 一、首次访问剪贴板的固定开销：约 +6MB（与内容无关）

| 读取的内容 | 读取前 | 第 1 次读后 | 第 2、3 次 |
|---|---|---|---|
| 200×150 图片（TIFF 120 KB） | 9.8 MB | 16.4 MB | 16.5 MB |
| **纯文本 15 字节** | 9.8 MB | **15.9 MB** | 16.0 MB |
| **完全空的剪贴板** | 9.8 MB | **15.2 MB** | 15.2 MB |

**读一个空剪贴板也涨 5.4MB** —— 所以这跟图片、跟内容大小都没关系，是
AppKit/CoreServices 在首次触碰 `NSPasteboard` 时的一次性初始化。

`vmmap` 佐证（读取前后 dirty 内存增量）：

```
CoreServices     0K → 2432K    ← 最大单项
MALLOC_SMALL   192K → 1040K
__TEXT         ...  →  +512K
TOTAL dirty   1952K → 5504K
```

耗时也印证：第 1 次读取 **3.9 ms**，第 2 次 **80 µs**——差 50 倍，典型的
惰性初始化。

**结论**：这 6MB 是进入门槛，付一次，之后不再增长。把它算进基线即可。

#### 二、图片解码的峰值：约为 RGBA 的 3 倍（随尺寸线性）

| 图片尺寸 | RGBA 理论大小 | 读后 RSS |
|---|---|---|
| 200×150 | 0.1 MB | 16.3 MB |
| 1000×750 | 2.9 MB | 24.7 MB |
| 2000×1500 | 11.4 MB | 50.4 MB |
| 4000×3000 | 45.8 MB | **153.4 MB** |

扣掉 ~15MB 基线后约为 RGBA 的 **3 倍**，因为解码时同时存在三份数据：
剪贴板 TIFF 原始字节 → `image` crate 解码出的中间图 → 转换后的 RGBA。

**归还是延迟的**（4000×3000 那次实测）：

```
t=  0s   153.4 MB
t= 30s   107.6 MB
t=120s    14.2 MB   ← 已回到基线
```

约 **2 分钟**内分配器归还给系统。所以刚复制完大图时看到上百 MB 属正常，
**判据是它会不会随复制次数继续叠加**——重复读同尺寸图片 RSS 完全持平
（50.5→50.5→50.5），泄漏则会线性上升。

#### 三、文件传输：与文件大小完全无关 ✅

这是最需要说明的一点，因为直觉容易反过来。**实测复制并传输 2 GB 文件**：

| 时刻 | 发送方 | 接收方 |
|---|---|---|
| 传输前 | 9.7 MB | 9.5 MB |
| 传输中（每 3 秒采样，共 36 秒） | 12–18 MB | 9–16 MB |
| 传输后 | 13.5 MB | 15.6 MB |

2 GB 文件全程内存**没超过 18 MB**，20.6 秒传完（约 104 MB/s），
接收文件字节数与源一致。原因有两层：

1. **剪贴板只存路径引用**，不存内容——2 GB 文件在剪贴板上时读取剪贴板
   的内存占用（16.1 MB）与空剪贴板（15.2 MB）几乎相同。
2. **传输是流式的**，固定 256 KB 缓冲
   （`filetransfer.rs` 的 `buf: vec![0u8; CHUNK_SIZE]`），
   与文件大小无关。

> **所以"复制几个 G 的文件会不会撑爆内存"——不会。** 文件路径的内存
> 是 O(1) 的。真正会随大小增长的只有**图片**，因为图片必须整张解码进内存
> 才能跨平台传输（剪贴板里的图片是数据不是引用，没有"路径"可用）。

#### 实用判据

| 现象 | 是否正常 |
|---|---|
| 启动后稳定在 10–16 MB | ✅ 正常（含首次访问剪贴板的 ~6MB） |
| 复制大图后飙到上百 MB，2 分钟内回落 | ✅ 正常 |
| 复制同一张图 N 次，RSS 持平 | ✅ 正常 |
| 传几 GB 文件，内存不动 | ✅ 正常（流式 + 路径引用） |
| **复制次数越多 RSS 越高，且不回落** | ❌ 才是泄漏，需排查 |

> 唯一可优化的是图片解码的 3 倍系数（改 `arboard` 读图路径，避免中间拷贝）。
> 但图片同步是低频操作、且 2 分钟内自动归还，收益有限，暂不动。
...
联调时踩过的，都不是缺陷，但表现都像 bug：

**1. 落地目录只保留最新代际** ——「日志显示接收成功，去目录里却找不到文件」

接收目录按代际编号（`recv/0000000000000009/`），
`cleanup_materialized_except` 只保留最新一代，之前的会被删。所以在核对
落地文件名时，若中途又传了别的文件，先前那份的目录已经没了。

> 规避：让待验文件成为**最后一次**文件传输；或核对完再继续测别的。
> 这个陷阱害我怀疑过"是不是文件根本没发出去"，差点去查一个不存在的问题。

**2. 超过 `max_bytes` 被静默跳过** ——「两边都没反应，日志也不报错」（已修）

默认上限 100 MiB，超限内容跳过时只在 **DEBUG** 级别打一行
`本地变化跳过 (TooLarge)`。只开 INFO 的话什么都看不到。

> 现在不会了：文件的上限改成接收侧的 `auto_fetch_bytes`，超限只是挂起等人
> 点「取回」，托盘上有一行、图标上有个角标、日志里有 INFO。文本/图片仍有
> 一条硬上限（`INLINE_MAX_BYTES`），但那是 100 MiB，正常内容碰不到。

**3. 断连窗口内发出的内容不会重发** ——「明明同步了，对方没收到」

日志会写 `已复制 [X] N 字节，但暂无对端连接`——内容没丢也没报错，
只是当时无人接收，重连后不会自动补发。

> 规避：核对 `对端已连接` / `对端已断开` 的时间戳，确认发送时刻确实在
> 连接窗口内。

排查"收不到"时的通用顺序：先看 `本地变化跳过 (...)` 括号里的原因
（`Sensitive` / `TooLarge` / `Echo` / `KindDisabled`），它直接说明为什么
没发；再看连接时间戳；最后才怀疑网络。
