...
mod loopback_smoke {
    ...
    /// 这台机器现在能不能**自发自收**组播。
    ///
    /// 判据必须是真的发一份再收一份，不能只看 `join_multicast_v4` 是否成功。
    /// GitHub 的 macOS runner 上实测（见 `multicast_capability_report`）：
    ///
    /// ```text
    /// 网卡：lo0 127.0.0.1 / en0 192.168.64.4
    /// join 成功：["lo0(127.0.0.1)", "en0(192.168.64.4)"]
    /// 发送 ok=false / 自收 ok=false
    /// ```
    ///
    /// 两个网卡都加进了组，**失败的是 `send_to` 本身**——沙箱不许发组播。
    /// 按 join 结果判断的话会得出"可用"，然后测试照样红。
    ///
    /// 这也说明跳过是环境所限而非我们的缺陷：若是"没在正确网卡上 join"，
    /// join 那一步就该有失败。
    ///
