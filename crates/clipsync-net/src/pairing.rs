...
///
/// 4 位十进制 = 1 万种组合。撑住它的**不是**有效期，而是
/// `MAX_FAILED_ATTEMPTS`：SPAKE2 让离线穷举不可能，但**在线**穷举实测有
/// 818 次/秒（见 `measure_online_guessing_rate`），十几秒就能跑完 1 万种。
/// 每会话 5 次上限把它压到 5/10000，且攻击者必须在会话窗口内够得着配对端口。
///
/// 这个代价换来的是"看一眼、敲四个数字"。
